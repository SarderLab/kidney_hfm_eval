...
def load_from_csv(csv_path, embeddings_dir):
    ...
    for _, row in df.iterrows():
        ...
        # # load .pt embedding
        # pt_path = os.path.join(embeddings_dir, fn.replace(".png", ".pt"))
...
def evaluate_knn(X_train, y_train, X_test, y_test, classes=None, n_neighbors=20):
    ...
    # --- ensure probs columns align to `classes` and pad missing classes with zeros ---
    if classes is None:
        classes = knn_classes

    if set(knn_classes) == set(classes):
        # Just reorder probs to match `classes`
        order = [knn_classes.index(c) for c in classes]
        probs = probs[:, order]
    else:
        # Some classes not seen in this training fold: create a full (n, C_all) matrix
        full = np.zeros((probs.shape[0], len(classes)), dtype=probs.dtype)
        for j, c in enumerate(knn_classes):
            k = classes.index(c)  # where this class sits in the global ordering
            full[:, k] = probs[:, j]
        probs = full

    vprint(f"[KNN] classes_={knn_classes} aligned_to={classes} probs_shape={probs.shape}", level=2)
...
def run_cross_validation(X, y, groups, sample_ids, classes, seeds=(0,1,2), n_splits=5):
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y, groups), start=1):
            ...
            # — kNN —
            kNN_m, y_true_kNN, y_prob_kNN = evaluate_knn(
                X_tr_s, y_tr, X_te_s, y_te, classes
            )
            print(y_true_kNN.shape, "y_true_kNN", y_prob_kNN.shape, "y_prob_kNN")
            records.append({
                **kNN_m,
                "Model": "kNN",
                "Seed": seed,
...
def run_kNN_probe(csv_file, emb_root, out_dir, models=None, n_splits=5, seeds=(0, 1, 2), bootstrap=1000):
    ...
    all_models = ["UNI", "UNI2-h", "Virchow", "Virchow2",
    ...
    for fm in models:
        ...
        cv_path = os.path.join(fm_outdir, f"cv_results_kNN_{fm}.csv")
        boot_path = os.path.join(fm_outdir, f"bootstrap_replicates_kNN_{fm}.csv")
        ci_path = os.path.join(fm_outdir, f"bootstrap_CI_kNN_{fm}.csv")
