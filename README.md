...
[![DOI](https://zenodo.org/badge/1077797961.svg)](https://doi.org/10.5281/zenodo.18394932)

This repository provides modularized Python scripts for benchmarking and validating histopathology foundation models (HFMs) on downstream tasks such as linear probing, k-nearest neighbor (kNN), ABMIL (classification and regression) evaluation.
...
Before running the feature extraction script, you must download the model weights. Place the downloaded weights in a directory on your system (e.g., /path/to/weights).

---

## Model Weights (Required)

This repository **does not ship pretrained weights**. You must **download model weights from Hugging Face** and place them in a single directory. Then, you will pass that directory path to the code via `BASE_MODEL_WEIGHT`.

### Download weights from Hugging Face

Download the required weights for the models you plan to use (UNI, Virchow, Hibou, H-optimus, SP22M/SP85M, Prov-Gigapath, etc.) from Hugging Face and save them locally.

### Expected directory layout

All weights should live inside **one directory**, and your code will receive that directory path.
...
**Screenshot example (your directory should look like this):**

![Model weights directory structure](directory.png)

> If your filenames differ from the ones expected in the code (e.g., different extensions or naming), rename them to match the expected names or update the corresponding loader in `model_builders.py`.

---

**Step 2:** Feature Extraction Script

Once the model weights are in place, you can proceed with the feature extraction. Create a Python script (e.g., run_feature_extraction.py) and insert the following code:
...
BASE_MODEL_WEIGHT = "/path/to/weights"  # Path to model weights
